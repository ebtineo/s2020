...
class Model{
   protected $conexion;
    ...
    public function editarAlimento($id, $n, $e, $p, $hc, $f, $g){
        $n = htmlspecialchars($n);
        $e = htmlspecialchars($e);
        $p = htmlspecialchars($p);
        $hc = htmlspecialchars($hc);
    ...
    public function borrarAlimento($i){
        $i = htmlspecialchars($i);
        $sql="delete from alimentos where id=".$i.";";
        $result = mysqli_query($this->conexion, $sql);
        return $result;
    }

    public function logarme($u, $p, $session){
        $u = htmlspecialchars($u);
        $p = htmlspecialchars($p);

        // variable para pasarla al controller
        $acceder = false;
